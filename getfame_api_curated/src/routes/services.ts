...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (ENV.CURATED_ONLY) {
      // No catalog but curated-only requested: return empty to avoid exposing full JAP list
      return res.json({ services: [], count: 0, curated: true });
    }

    // Fallback: non-curated filtered list (legacy behavior)
...
// raw passthrough for admin discovery
